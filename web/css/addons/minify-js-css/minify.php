...
// gwil
function populate_crush_config(){
    if( file_exists(K_COUCH_DIR.'addons/minify-js-css/crush_config.php') ){
        $cfg = array();

        require_once( K_COUCH_DIR.'addons/minify-js-css/crush_config.php' );
...
class MinifyJsCss{
    ...
    public static function minify_js_css( $params, $node ){
        global $FUNCS;
        ...
        //gwil
        if($filetype != 'css' && $filetype != 'csscrush' && $filetype != 'js'){die("ERROR: Tag \"".$node->name."\" - Must specify either 'css', 'csscrush' or 'js'.");}
        //gwil
        $crush = false;
        if ($filetype == 'csscrush') {
            $crush = true;
            $filetype = 'css';            
        }
        ...
        //$output_link = ($filepath) ? 'https://g8q3w7p7.hostrycdn.com/' . $filepath : '';
        ...
        if ($filetype == 'css'){ 
            // gwil
            if ($crush) {
                require_once( K_COUCH_DIR.'addons/minify-js-css/css_crush/CssCrush.php' );
                $output = $output.csscrush_string($content, populate_crush_config());
            } else {
        ...
        if ($filetype == 'js'){
            ...
            // gwil
            require_once( K_COUCH_DIR.'addons/minify-js-css/Minifier.php' );
