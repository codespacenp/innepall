<?php
    ...
    class KWatermark{
        ...
        static function watermark_handler( $params, $node ){
            ...
            $with = str_replace( "\\", "/", dirname(__FILE__) )."/$with";

            $at = strtolower( trim($at) );
            if( !in_array($at, array('tile', 'top_left', 'top_center', 'top_right', 'middle_left', 'middle', 'middle_right', 'bottom_left', 'bottom_center', 'bottom_right')) ){
                $at = 'bottom_right';
            }
        ...
        static function _create_watermarked_image( $image, $with, $at, $save_as ){
            ...
            if( $at == 'tile' ){
                ...
                $cntX = floor( $destW / $srcW );
                $cntY = floor( $destH / $srcH );
                $padX = floor( ($destW % $srcW) / 2 );
                $padY = floor( ($destH % $srcH) / 2 );
                imagealphablending( $dest, 1 );
                imagealphablending( $src, 1 );

                for( $y=0; $y<$cntY; $y++ ){
                    for( $x=0; $x<$cntX; $x++ ){
                        imagecopy( $dest, $src, $x * $srcW + $padX, $y * $srcH + $padY, 0, 0, $srcW, $srcH );
                    }
                }
